...
import vn.io.vutiendat3601.beatbuddy.catalog.constant.PlaylistScope;
...
public class BeatbuddyKeycloakAuthzClient {
    ...
    public Mono<ResourceRepresentation> createResource(ResourceRepresentation resourceRep) {
        return Mono.just(authzClient.protection().resource().create(resourceRep));
    }
    ...
    public Mono<PermissionTicketRepresentation> createPermissionTicket(String resourceId, PlaylistScope scope) {
        PermissionTicketRepresentation permissionTicketRep = new PermissionTicketRepresentation();
        permissionTicketRep.setResource(resourceId);
        permissionTicketRep.setScopeName(scope.instance().getName());
    ...
    public Mono<ResourceRepresentation> getResourceById(String id) {
        return Mono.just(authzClient.protection().resource().findById(id));
    }
